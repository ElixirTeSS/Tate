...
module AnnotationsVersionFu
  ...
  module ClassMethods
    def annotations_version_fu(options={}, &block)
      return if self.included_modules.include? Tate::AnnotationsVersionFu::InstanceMethods
      __send__ :include, Tate::AnnotationsVersionFu::InstanceMethods
      ...
      self.versioned_table_name         = options[:table_name]  || "tate_#{table_name_prefix}#{base_class.name.demodulize.underscore}_versions#{table_name_suffix}"
      ...
      class_eval do
        ...
        before_save :check_for_new_version if Tate::Engine.versioning_enabled
      end
