module Tate
  ...
  def process_value_adjustments(value_in)
    ...
    if value_out.is_a?(String)
      if Tate::Engine::attribute_names_for_values_to_be_downcased.include?(attr_name)
        value_out = value_out.downcase
      end
      if Tate::Engine::attribute_names_for_values_to_be_upcased.include?(attr_name)
        value_out = value_out.upcase
      end

      # Apply strip text rules
      Tate::Engine::strip_text_rules.each do |attr, strip_rules|
  ...
  def try_use_value_factory(value_in)
    ...
    if Tate::Engine::value_factories.has_key?(attr_name)
      return Tate::Engine::value_factories[attr_name].call(value_in)
    else
      return value_in
    end
  end
  ...
  def check_value
    ...
    if self.value.nil?
      ...
      attr_name = self.attribute_name.downcase
      if Tate::Engine::valid_value_types.has_key?(attr_name) &&
          !([ Tate::Engine::valid_value_types[attr_name] ].flatten.include?(self.value.class.name))
  ...
  def check_duplicate
    if ok_value_object_type?
      attr_name = self.attribute_name.downcase
      if Tate::Engine.attribute_names_to_allow_duplicates.include?(attr_name)
        return true
      else
  ...
  def check_limit_per_source
    attr_name = self.attribute_name.downcase
    if self.new_record? and Tate::Engine::limits_per_source.has_key?(attr_name)
      options = Tate::Engine::limits_per_source[attr_name]
      max = options[0]
  ...
  def check_content_restrictions
    if ok_value_object_type?
      ...
      if Tate::Engine::content_restrictions.has_key?(attr_name)
        options = Tate::Engine::content_restrictions[attr_name]
